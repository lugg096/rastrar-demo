...
export class PantallasComponent implements OnInit {
  ...
  async changeStatus(key, status) {
    ...
    try {
      await this._apiMongo.changeStatus(env.COLLECTION.object, env.TABLE_SIS.screen, key, status);
      this.getList();
  ...
  async delete(key) {
    ...
    try {
      await this._apiMongo.delete(env.COLLECTION.object, env.TABLE_SIS.screen, key);
